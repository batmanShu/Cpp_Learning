...
父类构造初始化：
+ 在创建子类对象时，为了初始化从父类中继承来的成员变量，编译器需要调用其父类的构造函数
+ 如果子类的构造函数没有显示地调用父类的构造函数，则默认调用父类的无参构造函数
+ 如果父类只声明了带参构造函数，子类需要在初始化列表中显式的调用父类的构造函数，如果该带参构造函数有默认值，那子类也可不显示调用
...
## C++三大特性作用
封装：目的在于将对象的使用者和设计者分开，以提高软件的可维护性和可修改性

继承：可以扩展已存在的代码，目的也是为了代码重用
